...
* **Live demo:** _add the GitHub Pages or Cloudflare Pages URL here after deploy_
...
- GitHub Pages / Cloudflare Pages / any static host is enough
...
- Static hosting (GitHub Pages, Cloudflare Pages, or equivalent) can run on a free tier for this tool.
...
- **`examples/outputs` from the three Salk clips (same tracker as the app):** `test50` ~97% coverage, 31 hole events; `test51` ~100% coverage, 12 events; `test53` ~83% coverage, 3 events. Target hole was **not** inferred — offline export used hole 1 as a placeholder, so primary latency is present only when that hole was investigated (`test50` ~124 s). A scientist must set the real target. `test51` coverage near 100% can include intervals where a hole-sized blob was still selected; those should be reviewed, not trusted as continuous visibility.
- **`test51` file timing:** MP4 `mdhd` timescale is 15000. Average fps from duration/frame count is about **15.005**, not integer 15. A leftover `stts` sample can make the file look VFR. The app prefers a dominant sample duration when ≥95% of frames share it, and uses media timestamps when tracking in the browser. OpenCV `CAP_PROP_FPS` also reported 15.005 — do not treat that as `15000/1001` without reading the boxes.
...
The build is static (`npm run build` → `dist/`). `base` is `./`, so GitHub Pages project sites work. A workflow is in `.github/workflows/pages.yml`. Enable Pages on the repository, or upload `dist/` to Cloudflare Pages / any static host.
