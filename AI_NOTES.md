...
Before treating a result as real: Vitest domain tests (timebase, cleanup, events, metrics, strategy, schema, CSV columns, dominant-`stts` duration); TypeScript + production build; the same tracker run on extracted frames from all three Salk MP4s; manual inspection of the official stills before choosing background subtraction. Sample MP4s were downloaded only into `.local-data/` and are not committed. Playwright covers the demo/review/export path when browsers are installed.
