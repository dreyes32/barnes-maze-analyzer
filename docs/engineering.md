...
`src/video/mp4Metadata.ts` parses `mdhd` / `stts`. `test51` must come out as `15000/1001`, not 15 and not 30. Tracking uses `HTMLVideoElement.currentTime` after each seek, not `frame/fps`.
